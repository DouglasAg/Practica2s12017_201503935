...
class Matriz:
    ...
    def matriznueva(self):
        self.raiz.siguiente=None
        self.raiz.abajo=None
...
# ESTE METODO ME BUSCA LA FILA CUANDO LA LETRAS COINCIDEN
    ...
    def graficar(self):
        nodograf=self.raiz.abajo
        nodograf2=nodograf
        graf=""
        while nodograf!=None:

            while nodograf2!=None:
                graf=graf+nodograf2.dato
                if nodograf2.siguiente!=None:
                    graf=graf + str(nodograf2.dato)+"->"+str(nodograf2.siguiente.dato)
                if nodograf2.abajo!=None:
                    graf=graf + str(nodograf2.dato)+"->"+str(nodograf2.abajo.dato)
                if nodograf2.arriba!=None:
                    graf=graf + str(nodograf2.dato)+"->"+str(nodograf2.arriba.dato)
                if nodograf2.anterior!=None:
                    graf=graf + str(nodograf2.dato)+"->"+str(nodograf2.anterior.dato)
                if nodograf2.atras!=None:
                    graf=graf + str(nodograf2.dato)+"->"+str(nodograf2.atras.dato)
                    graf=graf+srt(grafatras(nodograf2.atras))
                nodograf2=nodograf2.siguiente
            nodograf=nodograf.abajo
            nodograf2=nodograf

        nodcol=self.raiz.siguiente
        while nodcol!=None:
            if nodcol.abajo!=None:
                    graf=graf + str(nodocol.dato)+"->"+str(nodocol.abajo.dato)
            nodocol=nodocol.siguiente
        return graf 

    def grafatras(self,nodoa):
        nodoatras=nodoa
        grad=""
        while nodoatras!=None:
            grad=grad + str(nodoatras.dato)+"->"+str(nodoatras.adelante.dato)
            if nodoatras.atras!=None:
                grad=grad + str(nodoatras.dato)+"->"+str(nodoatras.atras.dato)
            nodoatras=nodoatras.atras
        return grad
